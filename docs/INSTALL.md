...
sudo /mnt/home/<user>/AsahiLocker/bin/luks-deploy.sh
```
...
```bash
sudo LUKS_PROFILE=fast /root/AsahiLocker/bin/luks-deploy.sh
```
...
roughly 1 s (fast), 3 s (moderate) and 11 s (aggressive). A 1 GiB argon2id unlock
...
```bash
sudo ./AsahiLocker/extras/install.sh
```

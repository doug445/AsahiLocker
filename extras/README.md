...
- `GRUB_UUIDS` tags volumes that GRUB itself must unlock (as opposed to the
  ...
  GRUB 2.12 has no argon2 support at all, and GRUB ≥ 2.13 caps argon2id memory at
  roughly 1 GiB. The root volume encrypted by this repo is unlocked by the
  initramfs, so it is unaffected.
